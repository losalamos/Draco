...
# Usage:
#
# add_component_executable( TARGET       "target name" EXE_NAME     "output executable name"
# TARGET_DEPS  "dep1;dep2;..." SOURCES      "file1.cc;file2.cc;..." HEADERS "file1.hh;file2.hh;..."
# VENDOR_LIST  "MPI;GSL" VENDOR_LIBS "${MPI_CXX_LIBRARIES};${GSL_LIBRARIES}" VENDOR_INCLUDE_DIRS
# "${MPI_CXX_INCLUDE_DIR};${GSL_INCLUDE_DIR}" FOLDER       "myfolder" PROJECT_LABEL "myproject42"
# NOEXPORT    - do not export target or dependencies to draco-config.cmake EXPORT_NAME - not
# currently used; but will be used to install a binary and add its information to
# <project>-config.cmake. NOCOMMANDWINDOW - On win32, do not create a command window (qt) )
#
# Example:
#
# add_component_executable( TARGET       Exe_draco_info EXE_NAME     draco_info TARGET_DEPS
# Lib_diagnostics SOURCES      "${PROJECT_SOURCE_DIR}/draco_info_main.cc" FOLDER       diagnostics )
#
...
# Usage:
#
# add_component_library( TARGET       "target name" LIBRARY_NAME "output library name" TARGET_DEPS
# "dep1;dep2;..." INCLUDE_DIRS "PUBLIC;$<BUILD_INTERFACE:${PROJECT_BINARY_DIR}>" SOURCES
# "file1.cc;file2.cc;..." HEADERS      "file1.hh;file2.hh;..." LIBRARY_NAME_PREFIX "rtt_"
# LIBRARY_TYPE "SHARED" VENDOR_LIST  "MPI;GSL" VENDOR_LIBS  "${MPI_CXX_LIBRARIES};${GSL_LIBRARIES}"
# VENDOR_INCLUDE_DIRS "${MPI_CXX_INCLUDE_DIR};${GSL_INCLUDE_DIR}" NOEXPORT )
#
# Example:
#
# add_component_library( TARGET       Lib_quadrature LIBRARY_NAME quadrature TARGET_DEPS
# "Lib_parser;Lib_special_functions;Lib_mesh_element" SOURCES      "${sources}" )
#
# Note: directories listed as VENDOR_INCLUDE_DIRS will be exported in the
# INTERFACE_INCLUDE_DIRECTORIES target property.
#
...
#
# register_scalar_test( TARGET "${compname}_${testname}_${numPE}" COMMAND
...
macro(register_scalar_test)
  ...
  set_tests_properties(
    ...
    PROPERTIES PASS_REGULAR_EXPRESSION
               "${addscalartest_PASS_REGEX}"
               FAIL_REGULAR_EXPRESSION
               "${addscalartest_FAIL_REGEX}"
               PROCESSORS
               "${num_procs}"
               WORKING_DIRECTORY
               "${PROJECT_BINARY_DIR}")
...
macro(register_parallel_test)
  ...
  if(DEFINED ENV{SLURM_CLUSTER_NAME}
     AND "$ENV{SLURM_CLUSTER_NAME}" STREQUAL "darwin"
     AND MPI_FLAVOR STREQUAL "openmpi"
     AND NOT "${MPIEXEC_EXECUTABLE}" MATCHES "smpi")
  ...
  set_tests_properties(
    ${rpt_TARGET}
    PROPERTIES PASS_REGULAR_EXPRESSION "${addparalleltest_PASS_REGEX}" FAIL_REGULAR_EXPRESSION
               "${addparalleltest_FAIL_REGEX}" WORKING_DIRECTORY "${PROJECT_BINARY_DIR}")
  ...
  if(addparalleltest_MPI_PLUS_OMP)
    ...
    set_tests_properties(${rpt_TARGET} PROPERTIES PROCESSORS "${numthreads}" LABELS "nomemcheck")
    unset(numthreads)
    unset(nnodes)
  ...
  if(ENABLE_MEMORYCHECK
    ...
    set_tests_properties(
      memcheck_${rpt_TARGET}
      PROPERTIES PASS_REGULAR_EXPRESSION
                 "${addparalleltest_PASS_REGEX}"
                 FAIL_REGULAR_EXPRESSION
                 "${addparalleltest_FAIL_REGEX}"
                 WORKING_DIRECTORY
                 "${PROJECT_BINARY_DIR}"
                 LABELS
                 "memcheck;${addparalleltest_LABEL}")
    if(NOT "${addparalleltest_RESOURCE_LOCK}none" STREQUAL "none")
      set_tests_properties(memcheck_${rpt_TARGET} PROPERTIES RESOURCE_LOCK
                                                             "${addparalleltest_RESOURCE_LOCK}")
    endif()
    if(NOT "${addparalleltest_RUN_AFTER}none" STREQUAL "none")
...
# Usage:
#
# add_scalar_tests( SOURCES "${test_sources}" [ DEPS    "${library_dependencies}" ] [ ENV
# "FOO=${CMAKE_BUILD_TYPE}" ] [ TEST_ARGS     "arg1;arg2" ] [ PASS_REGEX    "regex" ] [ FAIL_REGEX
# "regex" ] [ RESOURCE_LOCK "lockname" ] [ RUN_AFTER     "test_name" ] [ APPLICATION_UNIT_TEST ] )
#
# Options: APPLICATION_UNIT_TEST - (CI/CT only) If present, do not run the test under 'aprun'.
# ApplicationUnitTest based tests must be run this way.  Setting this option when DRACO_C4==SCALAR
# will reset any value provided in TEST_ARGS to be "--np scalar". LINK_WITH_FORTRAN - Tell the
# compiler to use the Fortran compiler for the final link of the test.  This is needed for Intel and
# PGI.
#
# --------------------------------------------------------------------------------------------------#
macro(add_scalar_tests test_sources)
  ...
  # Loop over each test source files: 1. Compile the executable 2. Register the unit test
  ...
  foreach(file ${addscalartest_SOURCES})
    ...
    if("${addscalartest_TEST_ARGS}none" STREQUAL "none")
      register_scalar_test(TARGET "${compname}_${testname}" COMMAND
                           "$<TARGET_FILE:Ut_${compname}_${testname}_exe>")
    else()
      set(iarg "0")
      foreach(cmdarg ${addscalartest_TEST_ARGS})
        math(EXPR iarg "${iarg} + 1")
        register_scalar_test(TARGET "${compname}_${testname}_arg${iarg}" COMMAND
                             "$<TARGET_FILE:Ut_${compname}_${testname}_exe>" CMD_ARGS "${cmdarg}")
      endforeach()
    endif()
  endforeach()
...
# Usage:
#
# add_parallel_tests( SOURCES "${test_sources}" DEPS    "${library_dependencies}" PE_LIST "1;2;4"
...
#
# DEPS            - CMake target dependencies. ENV             - Environment variables that will be
# set for the context of the running test. FAIL_REGEX      - If this regex exists in the output, the
# test will 'fail.' MPIFLAGS        - Extra options to pass to mpirun. PASS_REGEX      - This regex
# must exist in the output to produce a 'pass.' PE_LIST         - semi-colon delimited list of
# integers (number of MPI ranks). RESOURCE_LOCK   - Tests with this common string identifier will
# not be run concurrently. RUN_AFTER       - The argument to this option is a test name that must
# complete before the current test will be allowed to run SOURCES         - semi-colon delimited
# list of files. TEST_ARGS       - Command line arguments to use when running the test.
# --------------------------------------------------------------------------------------------------#
macro(add_parallel_tests)
  ...
  if("${addparalleltest_PE_LIST}none" STREQUAL "none")
    message(
      FATAL_ERROR
        "You must provide the keyword PE_LIST and a list containing the number of "
  ...
  # Loop over each test source files: 1. Compile the executable 2. Link against dependencies
  # (libraries)
...
targets for copying support files.")
  ...
  add_custom_target(Ut_${compname}_install_inputs_${Ut_${compname}_install_inputs_iarg} ALL
