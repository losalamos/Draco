...
#
# include( ApplicationUnitTest ) add_app_unit_test( DRIVER
# ${CMAKE_CURRENT_SOURCE_DIR}/tDracoInfo.cmake APP
# $<TARGET_FILE_DIR:Exe_draco_info>/$<TARGET_FILE_NAME:Exe_draco_info> LABELS nomemcheck )
#
# Optional Parameters: GOLDFILE      - Compare the output from APP against this file. STDINFILE -
# APP expects interactive input, use data from this file. WORKDIR       - APP must be run from this
# directory. BUILDENV FAIL_REGEX LABELS        - E.g.: nomemcheck, nr, perfbench PASS_REGEX PE_LIST
# - How may mpi ranks to use "1;2;4" RESOURCE_LOCK - Prevent tests with the same string from running
# concurrently. RUN_AFTER     - Run the named tests before this test is started. TEST_ARGS     -
# optional papmeters that will be given to APP.

# The above will generate a test with data similar to this:
#
# add_test( NAME ${ctestname_base}${argname} COMMAND ${CMAKE_COMMAND} -D APP=${aut_APP} -D
# ARGVALUE=${argvalue} -D WORKDIR=${aut_WORKDIR} -D TESTNAME=${ctestname_base}${argname} -D
# DRACO_CONFIG_DIR=${DRACO_CONFIG_DIR} -D
# DRACO_INFO=$<TARGET_FILE_DIR:Exe_draco_info>/$<TARGET_FILE_NAME:Exe_draco_info> -D
# STDINFILE=${aut_STDINFILE} -D GOLDFILE=${aut_GOLDFILE} -D RUN_CMD=${RUN_CMD} -D numPE=${numPE} -D
# PROJECT_BINARY_DIR=${PROJECT_BINARY_DIR} -D PROJECT_SOURCE_DIR=${PROJECT_SOURCE_DIR} ${BUILDENV}
# -P ${aut_DRIVER} ) ) set_tests_properties( diagnostics_draco_info PROPERTIES
# PASS_REGULAR_EXPRESSION Passes FAIL_REGULAR_EXPRESSION Fails LABELS nomemcheck )
...
#
# Returns: APP       - cleaned up path to executable STDINFILE - cleaned up path to intput file.
# BINDIR    - Directory location of binary file PROJECT_BINARY_DIR - Parent directory of BINDIR
# GOLDFILE  - cleaned up path to gold standard file. OUTFILE   - Output filename derived from
# GOLDFILE. ERRFILE   - Output(error) filename derived from GOLDFILE.
#
# if VERBOSE is set, also echo input values. APP       - path name for executable. STDINFILE -
# optional input file GOLDFILE  - optional gold standard file.
# ---------------------------------------------------------------------------##
macro(aut_setup)
...
")
  ...
  if(FALSE)
    ...
    # Capture all the output to log files: - before we create the file, extract some lines that we
    # want to exclude: 1. Aprun inserts this line: "Application 12227386 resources: utime ~0s, stime
    # ~0s, ..."
    ...
    # unset(testout)
    set(testout ${newout})
    # join( "${newout}" "\n" testout ) string( REGEX REPLACE "${Esc};" "\n" testout ${testout} )
  endif()

  # now write the cleaned up file
  file(WRITE ${OUTFILE} "${testout}")
  # [2015-07-28 KT] not sure we need to dump stderr values right now file( WRITE ${ERRFILE}
  # ${testerr} )
...
#
# add_app_unit_test( DRIVER    ${CMAKE_CURRENT_SOURCE_DIR}/tstAnaheim.cmake APP
# $<TARGET_FILE_DIR:Exe_anaheim>/$<TARGET_FILE_NAME:Exe_anaheim> BUILDENV
# "GDIFF=$<TARGET_FILE_DIR:Exe_gdiff>/$<TARGET_FILE_NAME:Exe_gdiff>;PGDIFF=$<TARGET_FILE_DIR:Exe_pgdiff>/$<TARGET_FILE_NAME:Exe_pgdiff>"
#
# ---------------------------------------------------------------------------##
macro(aut_gdiff infile)

  # Sanity checks 1. Must be able to find gdiff.  Location is specified via BUILDENV
  # "GDIFF=$<TARGET_FILE_DIR:Exe_gdiff>/$<TARGET_FILE_NAME:Exe_gdiff>" 2. Input file ($1) must exist
