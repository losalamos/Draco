...
macro(setupPython)
  ...
  # Python_Interpreter_FOUND - Was the Python executable found Python_EXECUTABLE  - path to the
  # Python interpreter
...
#
# This module sets the following variables: - BLAS_FOUND   - set to true if a library implementing
# the BLAS interface is found - LAPACK_FOUND - set to true if a library implementing the LAPACK
# interface is found
#
# Provides import targets: - LAPACK::LAPACK - BLAS::BLAS
#
# Providers: - Linux - use spack to install netlib-lapack, openblas, or mkl,
# https://github.com/spack/spack - Windows - Use vcpkg, https://github.com/microsoft/vcpkg, or clone
# and build from sources, https://github.com/KineticTheory/lapack-visualstudio-mingw-gfortran
# --------------------------------------------------------------------------------------------------
...
macro(setupGSL)
  ...
  if(NOT TARGET GSL::gsl)
    ...
    # 1. Config mode. If CMAKE_PREFIX_PATH contains a GSL install prefix directory and the file
    #   gsl-config.cmake is found somewhere in this installation tree, then the targets defined by
    #   gsl-config.cmake will be used.
  ...
  if(NOT TARGET GSL::gsl) # if option #1 was successful, skip this.
    ...
    # 1. pkg-config mode (Linux) IF GSL_ROOT_DIR isn't set, look for the binary 'gsl-config' in
    #   $PATH. If found, run it to discover and set GSL_ROOT_DIR that will be used in method #3.
    ...
    # 1. Module mode. Locate GSL by using the value of GSL_ROOT_DIR or by looking in standard
    #   locations. We add 'REQUIRED' here because if this fails, then we abort the built.
  ...
  # If successful in finding GSL, provide some information for the vendor summary reported by
  # src/CMakeLists.txt.
  if(TARGET GSL::gsl)
    # =============================================================================
...
macro(setupLIBQUO)
  ...
  if(NOT TARGET LIBQUO::libquo AND MPI_C_FOUND)
    ...
    set_package_properties(
      ...
      DESCRIPTION
        "A runtime library that aids in accommodating thread-level
...
# ------------------------------------------------------------------------------
# Helper macros for setup_global_libraries() Assign here the library version to be used.
# ------------------------------------------------------------------------------
macro(setVendorVersionDefaults)
  # Set the preferred search directories(ROOT)

  # Check that VENDOR_DIR is defined as a cache variable or as an environment variable. If defined
  # as both then take the environment variable.
  ...
  if(IS_DIRECTORY "${VENDOR_DIR}")
    set(VENDOR_DIR
        ${VENDOR_DIR}
        CACHE PATH "Root directory where CCS-2 3rd party libraries are located." FORCE)
  endif()

  # Import environment variables related to vendors 1. Use command line variables
  # (-DLAPACK_LIB_DIR=<path> 2. Use environment variables ($ENV{LAPACK_LIB_DIR}=<path>) 3. Try to
  # find vendor in $VENDOR_DIR 4. Don't set anything and let the user set a value in the cache after
  # failed 1st configure attempt.
  ...
  if(NOT DEFINED RANDOM123_INC_DIR AND IS_DIRECTORY ${VENDOR_DIR}/Random123-1.08/include)
...
# ------------------------------------------------------------------------------
# This macro should contain all the system libraries which are required to link the main objects.
# ------------------------------------------------------------------------------
