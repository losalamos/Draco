...
#
# Returns (as cache variables) - MPI_VERSION - MPI_FLAVOR = {openmpi, mpich, cray, spectrum,
# mvapich2, intel}
# --------------------------------------------------------------------------------------------------#
...
# Query CPU topology
#
# Returns: MPI_CORES_PER_CPU MPI_CPUS_PER_NODE MPI_PHYSICAL_CORES MPI_MAX_NUMPROCS_PHYSICAL
# MPI_HYPERTHREADING
#
# See also: - Try running 'lstopo' for a graphical view of the local topology or 'lscpu' for a text
# version. - EAP's flags can be found in Test.rh/General/run_job.pl (look for $other_args).  In
...
macro(query_topology)
  ...
  # hyper-threading is enabled (if logical != physical cores) - cmake_host_system_information(RESULT
  # MPI_PHYSICAL_CORES QUERY NUMBER_OF_PHYSICAL_CORES) - cmake_host_system_information(RESULT
  # MPI_LOGICAL_CORES  QUERY NUMBER_OF_LOGICAL_CORES)
...
macro(setupOpenMPI)
  ...
  # Notes: - For PERFBENCH that use Quo, we need '--map-by socket:SPAN' instead of '-bind-to none'.
  # The 'bind-to none' is required to pack a node. - Adding '--debug-daemons' is often requested by
  # the OpenMPI dev team in conjunction with 'export OMPI_MCA_btl_base_verbose=100' to obtain debug
  # traces from openmpi.
  ...
  # Spectrum-MPI on darwin Limit communication to on-node via '-intra sm' or 'intra vader'
...
macro(setupCrayMPI)
  ...
  # --------------------
  # -N        limit job to a single node. --gres=craynetwork:0 This option allows more than one srun
  # to be running at the same time on the Cray. There are 4 gres “tokens” available. If unspecified,
  # each srun invocation will consume all of them. Setting the value to 0 means consume none and
  # allow the user to run as many concurrent jobs as there are cores available on the node. This
  # should only be specified on the salloc/sbatch command.  Gabe doesn't recommend this option for
  # regression testing. --vm-overcommit=disable|enable Do not allow overcommit of heap resources. -p
  # knl    Limit allocation to KNL nodes. srun options:
  # --------------------
  ...
  # background tasks to have them run simultaneously, this option is required to be set or they will
...
macro(setupMPILibrariesWindows)
  ...
  # tested: 2017-08-31)
  #
  # 1.) You need MinGW/MSYS. Please make sure that the Devel tools are installed. 2.) Download and
  # install Microsoft's MPI. You need the main program and the SDK. 3.) In the file
  # %MSMPI_INC%\mpif.h, replace INT_PTR_KIND() by 8 4.) Create a MSYS version of the MPI library: cd
  # %TEMP% copy c:\Windows\System32\msmpi.dll msmpi.dll gendef msmpi.dll dlltool -d msmpi.def -l
  # libmsmpi.a -D msmpi.dll del msmpi.def copy libmsmpi.a %MSMPI_LIB32%/libmsmpi.a
