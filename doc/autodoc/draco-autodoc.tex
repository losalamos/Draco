...
special comment keywords. Both html (with links)
...
Again, ee the manual for more details.
...
\end{verbatim}
The documentation is then produced and stored in the
html and \LaTeX\ directories.
...
\item[\ttfamily PROJECT\_NUMBER] This tag gives the release number of
  ...
  See  section \S~\ref{sec:spcom} for details on release numbers.
...
\draco.  We feel that this is too restrictive. Also, we feel that
the package documentation should be available to the user immediately
upon checkout. We will consider the \texttt{autodoc} directory as 
a package unto itself that depends on all the other packages in 
\draco. As such it will have \verb+autodoc-#_#_#+, \verb+last_stable+, and
\verb+draco+ tags as described in Ref~\cite{xtm:9936}. The PROJECT\_NUMBER
definition in \texttt{draco/autodoc/draco.config} should be set to
the value given in \verb+autodoc-#_#_#+.
...
Also the PROJECT\_NUMBER definition in the \texttt{draco/autodoc/config.draco}
file will need to be periodically updated to reflect the current 
\texttt{autodoc} version number.
The resulting output should be tagged and committed to
CVS using the same procedures as for the source code as detailed in
 Ref~\cite{xtm:9936}. Note that
...
\item Using the \doxy\
 \verb+\page+ command, each package should provide a brief overview of the 
 purpose, design and contents of the package  in the package
...
to the \texttt{<system>/autodoc/} html header and footer files. 
...
assure that their html documentation links are kept up to date. 
...
For additional samples, both the \rng\ and \timestep\ packages have been
autodocumented.  
