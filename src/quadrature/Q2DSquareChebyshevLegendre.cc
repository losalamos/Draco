    ...
    : Quadrature( sn_order_, norm_ ), numAngles (sn_order_ * sn_order_)
{
    ...
    Require ( snOrder > 0 );
    Require ( norm > 0.0 );
//    Insist ( snOrder == 8, "Square CL Quad hand-coded for S8 only." );
...
// NOTE: this aligns the gauss points with the y-axis (z-axis in cylindrical coords)
    ...
    for (int i=0; i<snOrder; ++i)
    {
        ...
        for (int j=0; j<snOrder; ++j)
        {
            ...
            mu[angle]  = xmu;
            eta[angle] = xsr*cos(rtt_units::PI*(2.0*j+1.0)/snOrder/2.0);
