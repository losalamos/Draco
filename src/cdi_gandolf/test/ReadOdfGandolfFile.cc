...
void printSyntax()
{
		cerr << "syntax: ReadOdfGandolfFile [--help] [--bands n | --mg] [--analyze | --printc | --printtable | --collapse] [--model (r[osseland] | p[lanck])] [--reaction (t[otal] | a[bsorption] | s[cattering])] [-d density -t temperature] fileName" << endl;
}
//---------------------------------------------------------------------------//
...
int main(int argc, char *argv[])
{
	if (argc <= 1)
	{
		cerr << "Must have at least one input argument (the gandolf file)." << endl;
		printSyntax();
		...
		{
			cout << "**** ReadOdfGandolfFile Test: PASSED" << endl;
		}
		cout <<     "*********************************************" << endl;
		cout << endl;
		return 1;
	...
	string gandolfFileName = argv[argc - 1];
	...
	// data needed to load the Gandolf file properly
	int numBands = 0;
	...
	for (int arg = 1; arg < argc - 1; arg++)
	{
		...
		else if (currentArg == "--model")
		{
			...
			{
				cerr << "Unrecognized model option '" << argv[arg] << "'" << endl;
	...
	spGandOpacity = new GandolfOdfmgOpacity(file, matID, model, reaction, numBands);

	cerr	<< "Successfully read Gandolf file \"" << gandolfFileName 
			<< "\"." << endl;
...
void collapseOpacities(SP_Goo spGandOpacity, double temperature, double density)
{
	...
	Check(numBands + 1 == bandBoundaries.size());
