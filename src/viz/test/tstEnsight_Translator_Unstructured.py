...
try:
  ...
  for var in vars:
    ...
    tEnsight_Translator.diff_two_files("PROJECT_BINARY_DIR", \
      "unstr2d_testproblem_serial_{0}_ensight/{1}/data.0001".format(numpe,var), "PROJECT_SOURCE_DIR", \
      "unstr2d_bench_serial/{0}.0001".format(var))
    tEnsight_Translator.diff_two_files("PROJECT_BINARY_DIR", \
      "unstr2d_testproblem_parallel_{0}_ensight/{1}/data.0001".format(numpe,var), "PROJECT_SOURCE_DIR", \
  ...
  if tEnsight_Translator.is_little_endian():
    ...
    for var in vars:
      ...
      tEnsight_Translator.diff_two_files( "PROJECT_BINARY_DIR", \
        "unstr2d_testproblem_serial_{0}_binary_ensight/{1}/data.0001".format(numpe,var), \
        "PROJECT_SOURCE_DIR", "unstr2d_bench_serial/{0}.bin.0001".format(var), \
        diff_name=diff_prog)
      tEnsight_Translator.diff_two_files( "PROJECT_BINARY_DIR", \
        "unstr2d_testproblem_parallel_{0}_binary_ensight/{1}/data.0001".format(numpe,var), \
        "PROJECT_SOURCE_DIR", "unstr2d_bench_parallel_{0}/{1}.bin.0001".format(numpe,var), \
        diff_name=diff_prog)
