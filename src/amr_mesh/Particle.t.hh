...
bool Particle<MT>::surface(const MT &mesh, int face)
{
  ...
  // determine the next cell
    int next_cell;
    // the next cell is either at the same generation level or going from
    // a refined region to a coarse region
    if (mesh.num_adj(cell, face) == 1)
        next_cell = mesh.next_cell(cell, face, 1);
    // going from a coarse region to a refined region.
    else
    {
        // determine the "opposed" face number
        int adj_face = 2 * mesh.get_Coord().get_sdim() - face + 1;
	int adj_cell = 1;
	next_cell = mesh.next_cell(cell, face, adj_cell);
	while (adj_cell <= mesh.num_adj(cell, face) && 
	       !mesh.check_on_face(r, next_cell, adj_face))
	{
	      ++adj_cell;
	      next_cell = mesh.next_cell(cell, face, adj_cell);
	}
        Insist(adj_cell <= mesh.num_adj(cell, face),
	       "Refined generation next cell number not found!");
    }
