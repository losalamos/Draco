...
void Parallel_Source_Init<MT,PT>::calc_init_ecen()
{
	...
	ecen(cell) = rtt_mc::global::a * ecen.get_Mesh().volume(cell) *
						...
						const Mat_State<MT> &state)
{
	...
	{
	    int face_low  = mesh.get_Coord().get_dim() - coord + 1;
	    int cell_low = mesh.next_cell(cell, face_low, 1);
	    if (mesh.num_adj(cell, face_low) == 1)
		t4_low = pow(state.get_T(cell_low), 4);
	    else
	    {
	        // calculate an average temperature if this face is adjacent 
	        // to multiple cells. 
		t4_low = 0.;
		for (int a = 1; a<= mesh.num_adj(cell, face_low); a++)
		    t4_low += state.get_T(mesh.next_cell(cell, face_low, a));
		t4_low /= static_cast<double>(mesh.num_adj(cell, face_low));
		t4_low = pow(t4_low,4);
	    }

	    int face_high = mesh.get_Coord().get_dim() + coord;
	    int cell_high = mesh.next_cell(cell, face_high, 1);
	    if (mesh.num_adj(cell, face_high) == 1)
	    ...
	    else
	    {
	        ...
	        // to multiple cells. 
		t4_high = 0.;
		for (int a = 1; a<= mesh.num_adj(cell, face_high); a++)
		    t4_high += state.get_T(mesh.next_cell(cell, face_high, a));
		t4_high /= static_cast<double>(mesh.num_adj(cell, face_high));
		t4_high = pow(t4_high,4);
