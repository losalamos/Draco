 ...
 *    mesh files in the RTT format. The \ref amr_overview page presents a 
 *    summary of the capabilities provided within the namespace.
 */
namespace rtt_amr 
{
/*!
 ...
 *    input file and a continuous adaptive refinement mesh file using the \ref
 *    rtt_format_defined (the latter file can be generated by the mesh 
 *    generation codes ICEM and X3D). An rtt_format::RTT_Format class object
 *    is instantiated and returned by the parsing member function and actually 
 *    reads the designated mesh file. The RTT_Format class is equipted with 
 *    numerous member functions that allow accessing the mesh data and this 
 *    object also automatically determines the connectivity of the mesh nodes,
 *    sides, and cells. In C++ the CAR_CU_Interface parsing member functions 
 ...
 *    Mesh field classes can also be constructed and accessed at will using 
 *    either programming language. All mesh field accessor functions are also
 *    shadowed. 
 *
