 ...
 * other words, this stream is most suitable for TL;DR type messages to a file.
 *
 * Memory allocated for buffering is retained between calls unless the
 * shrink_to_fit function is called, which releases all memory not needed for
 * the current buffered output. In typical usage, this means the stream will
 * grow its buffer size silently until it is big enough for the largest blocks
 * of output being generated, then remain there through the rest of the scope
 * containing the stream, unless shrink_to_fit is called.
 *
