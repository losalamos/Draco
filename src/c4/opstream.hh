 ...
 * is printed to cout, by MPI rank. In other words, all output inserted to pout
 * by rank 0 since the last call to send() is printed to cout, then all output
 * inserted to pout by rank 1 is printed to cout, and so on for all
 * processors. There is no requirement that any processor have actually inserted
 * any output to pout since the last cal to send(), nor does the output from any
 ...
 * between send() calls, memory consumption might become significant if very
 * large blocks of output are generated by MPI ranks between calls to send(). In
 * other words, this is for TL;DR type messages to the console.
 *
 * Memory allocated for buffering is retained between calls unless the
 * shrink_to_fit function is called, which returns all memory not needed for the
 * current buffered output. In typical usage, this means pout will grow its
 * buffer size silently until it is big enough for the largest blocks of output
 * being generated, then remain there through the rest of the run, unless
 * shrink_to_fit is called.
 *
 * Any output remaining in the buffer when the buffer goes out of scope and is
 * destroyed will be discarded. The alternative, of doing a final send() as part
 * of the destructor, risks propagating an exception out of the destructor,
