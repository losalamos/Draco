 ...
 * procedure reduces of the norm of the set of functions along the
 * ray defined by the starting point and search direction.  It is used mainly
 * as an auxiliary to various quasi-Newton methods for solving sets of
 * nonlinear equations.
 *
 * Because this function is primary a helper function for quasi-Newton
 * methods, no great accuracy requirement is placed on the convergence
 * to the minimum.  Since the Newton step is always tried first, we should
 * get rapid convergence just by accepting this step if we are close enough
 * to the root.  If not, it is sufficient that we have *some* improvement,
 * and \c ALF characterizes how much is required.
 *
 ...
 * \param p
 * Search direction.  Normally p+xold will be the estimated location
 * of the minimum and will always be tried first.
 * \param x
 * On exit, contains the new estimate of the minimum location.
 * \param f
 * On exit, contains the new estimate of the minimum norm.
 * \param check
 * On exit, set to true if the search failed.
 * \param fvec
 * Values of the functions at the new minimum point.
 * \param vecfunc
 ...
 * an exception.
 * \param min_lambda
 * Value of lambda (line search parameter) at which to give up
 */
...
{    
    ...
    for (unsigned i=1; i<n; i++) slope += g[i]*p[i];
    if (slope>=0)
    {
	check = true;
	return;
	...
	for (unsigned i=0; i<n; i++)
	{
	    ...
	    if (x[i] != xold[i]) lambda_too_small = false;
	}
	...
	try 
	{
		...
		else
		{
			...
			lambda = min(0.5*lambda,
				     max(0.1*lambda, (sqrt(det)-b)/(3*a)));
		}
	    }
	}
	catch(...)
