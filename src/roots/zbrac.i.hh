 ...
 *
 * \param[in] func
 * Function whose root is to be bracketed
 * \param[in,out] x1
 * Lower end of search interval
 * \param[in,out] x2
 * Upper end of search interval
 *
 ...
 * Indicates that no root could be bracketed, perhaps because the function
 * has none.
 *
 * \note zbrac cannot fail to bracket a root or roots of an odd polynomial.
 * It is otherwise impossible to guarantee success. It can only be guaranteed
 * that, if zbrac returns rather than throwing std::domain_error, then the
 * interval <code>[x1,x2]</code> contains at least one root or singularity of
 * the function.
 */
