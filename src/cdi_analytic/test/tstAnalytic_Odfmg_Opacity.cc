	  									...
	  									vector<double> ref)
{
	...
	for (int group = 0; group < sigma.size(); group++)
	{
		for (int band = 0; band < sigma[group].size(); band++)
		{
...
void odfmg_test()
{
	...
	// load groups * bands opacities; all bands inside each group should
	// be the same
...
void test_CDI()
{
	...
	// scalar density and temperature
	vector< vector<double> > sigma = cdi.odfmg(rtt_cdi::ANALYTIC, 
