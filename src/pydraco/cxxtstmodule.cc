...
PyObject *foo( Object self, Tuple args )
{
    cout << "In foo.\n";

//     throw RuntimeError( "freaking out!" );
//     throw "really really freaking out!";
//     throw int(2);
...
// Create the module and add the functions.
    ...
    cout << "executing initcxxtst" << endl;

    vector<CXXMethodDef< CXXFunction > *> cxxtst_methods;

    CXXMethodDef< CXXFunction > *fl =
        new CXXMethodDef< CXXFunction >( "foo", foo, "foo doc" );
//         CXXMethodDef< CXXFunction > fl = {"foo", foo, "foo doc" };

    cxxtst_methods.push_back( fl );

    PyObject *m = Py::XInitmodule4( "cxxtst", cxxtst_methods,
                                    cxxtst_module_documentation,
//                                     NULL, PYTHON_API_VERSION );
                                    Nothing(), PYTHON_API_VERSION );
