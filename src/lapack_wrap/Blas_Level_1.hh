//----------------------------------*-C++-*----------------------------------//
/*!
 * \file   lapack_wrap/Blas_Level_1.hh
 * \author Thomas M. Evans
 * \date   Wed Sep  4 17:21:47 2002
 * \brief  Level 1 Blas functions.
 */
//---------------------------------------------------------------------------//
// $Id$
//---------------------------------------------------------------------------//

#ifndef __lapack_wrap_Blas_Level_1_hh__
#define __lapack_wrap_Blas_Level_1_hh__
		      ...
		      int          increment_y)
{
    ...
    SCOPY(&N, const_cast<float *>(x), &increment_x, y, &increment_y);
}
		      ...
		      int           increment_y)
{
    ...
    DCOPY(&N, const_cast<double *>(x), &increment_x, y, &increment_y);
}
 ...
 */
template<class T>
inline void blas_copy(const std::vector<T> &x,
		      int                   increment_x,
		      std::vector<T>       &y,
		      int                   increment_y)
{
    ...
    blas_copy(static_cast<int>(x.size()), &x[0], increment_x, 
	      &y[0], increment_y);
}
		      ...
		      int          increment_y)
{
    ...
    return SDOT(&N, const_cast<float *>(x), &increment_x,
		       ...
		       int           increment_y)
{
    ...
    return DDOT(&N, const_cast<double *>(x), &increment_x, 
 ...
 */
template<class T>
inline T blas_dot(const std::vector<T> &x,
		  int                   increment_x,
		  ...
		  int                   increment_y)
{
    ...
    return blas_dot(static_cast<int>(x.size()), &x[0], increment_x, 
		    &y[0], increment_y);
}
		      ...
		      int          increment_y)
{
    ...
    SAXPY(&N, &alpha, const_cast<float *>(x), &increment_x, y, &increment_y);
}
		      ...
		      int           increment_y)
{
    ...
    // do a double precision axpy
    DAXPY(&N, &alpha, const_cast<double *>(x), &increment_x, y, &increment_y);
}
		      ...
		      int                   increment_y)
{
    ...
    blas_axpy(static_cast<int>(x.size()), alpha, &x[0], increment_x, 
	      &y[0], increment_y);
}
		       ...
		       int          increment_x)
{
    ...
    float nrm2 = SNRM2(&N, const_cast<float *>(x), &increment_x);
    Check (nrm2 >= 0.0);
    return nrm2;
}
			...
			int           increment_x)
{
    ...
    double nrm2 = DNRM2(&N, const_cast<double *>(x), &increment_x);
    Check (nrm2 >= 0.0);
    return nrm2;
}
 ...
 */
template<class T>
inline T blas_nrm2(const std::vector<T> &x,
		   int                   increment_x)
{
    Check (typeid(T) == typeid(float) || typeid(T) == typeid(double));
...
#endif                          // __lapack_wrap_Blas_Level_1_hh__

//---------------------------------------------------------------------------//
//                              end of lapack_wrap/Blas_Level_1.hh
//---------------------------------------------------------------------------//
